...
typedef struct VideoState {
    ...
    int overlay; // TRUE when the video is overlayed on top of something and can be transparent
    int autoresize; // TRUE when video should be automatically resized to fit window size
} VideoState;
...
static int default_width  = 1280; // RONEN - changed from 640
static int default_height = 720; // RONEN - changed from 480
...
static int find_stream_info = 0; // RONEN - changed from 1, othwerwise UI stream does not show until more data becomes available
...
static int realloc_texture(SDL_Texture **texture, Uint32 new_format, int new_width, int new_height, SDL_BlendMode blendmode, int init_texture)
{
    ...
    if (!*texture || SDL_QueryTexture(*texture, &format, &access, &w, &h) < 0 || new_width != w || new_height != h || new_format != format) {
        ...
        if (SDL_SetTextureBlendMode(*texture, blendmode) < 0)
            return -1;
        // RONEN - use this to create transparency when is->overlay
        // if (SDL_SetTextureBlendMode(*texture, SDL_BLENDMODE_BLEND) < 0) // RONEN
        //     return -1;            
        // if (SDL_SetTextureAlphaMod(*texture, 100) < 0) // RONEN
        //     return -1;
        if (init_texture) {
...
static void video_image_display(VideoState *is)
{
    ...
    // RONEN - added this instead of the code commented in video_display() and change the alpha value of 255 to 0
    SDL_Rect scr_rect = { is->xleft, is->ytop, is->width, is->height };
    SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND);
    SDL_SetRenderDrawColor(renderer, 0, 0, 0, is->overlay ? SDL_ALPHA_TRANSPARENT : SDL_ALPHA_OPAQUE);
    SDL_RenderFillRect(renderer, &scr_rect);
...
void stream_close(VideoState *is)
{
    ...
    // RONEN
    av_dict_free(&is->swr_opts);
    av_dict_free(&is->sws_dict);
    av_dict_free(&is->format_opts);
    av_dict_free(&is->codec_opts);
    av_dict_free(&is->resample_opts);
...
{
    // RONEN - replaced if (!is->width)
    // This is because we want to set the width
...
static int configure_video_filters(AVFilterGraph *graph, VideoState *is, const char *vfilters, AVFrame *frame)
{
    ...
    while ((e = av_dict_get(is->sws_dict, "", e, AV_DICT_IGNORE_SUFFIX))) {
...
static int configure_audio_filters(VideoState *is, const char *afilters, int force_output_format)
{
    ...
    while ((e = av_dict_get(is->swr_opts, "", e, AV_DICT_IGNORE_SUFFIX)))
...
static int read_thread(void *arg)
{
    ...
    // RONEN - commented since we do not use the global variable seek_by_bytes
    // if (seek_by_bytes < 0)
    ...
    for (;;) {
        ...
        if (is->paused &&
                (!strcmp(ic->iformat->name, "rtsp") ||
                 (ic->pb && input_filename && !strncmp(input_filename, "mmsh:", 5)))) {
...
static void refresh_loop_wait_event(VideoState *is[], size_t count, SDL_Event *event) {
    ...
    while (1) {
        ...
        // RONEN - replaced with code below that handles multiple videos
...
static void event_loop(VideoState *cur_stream, VideoState *other_stream)
{
    SDL_Event event;
    ...
    for (;;) {
        double x;
        // refresh_loop_wait_event(cur_stream, other_stream, &event);
        switch (event.type) {
        case SDL_KEYDOWN:
        ...
        case SDL_WINDOWEVENT:
            switch (event.window.event) {
                case SDL_WINDOWEVENT_SIZE_CHANGED:
                    ...
                    {
                        other_stream->width  = event.window.data1;
                        other_stream->height = event.window.data2;
                    }
                    // RONEN - replace the above with the below if the width and height of the video should not change based on window size
                    ...
                    if (other_stream != NULL && other_stream->vis_texture) {
                        SDL_DestroyTexture(other_stream->vis_texture);
                        cur_stream->vis_texture = NULL;
                    }
                case SDL_WINDOWEVENT_EXPOSED:
                    cur_stream->force_refresh = 1;
                    if (other_stream != NULL)
                        other_stream->force_refresh = 1;
            }
            break;
        case SDL_QUIT:
        case FF_QUIT_EVENT:
...
int vp_get_video_size(vp_handle_t handle, int *width, int *height)
{
    ...
    {
        *width = is->video_st->codecpar->width;
        *height = is->video_st->codecpar->height;
        return 0;
    }
    return -1;
}
...
{
    VideoState *is = (VideoState *) handle;
    return is->ic && is->ic->pb ? is->ic->pb->seekable != 0 : 0;
}

double vp_get_duration(vp_handle_t handle)
{
    VideoState *is = (VideoState *) handle;
    return is->ic ? is->ic->duration / (double) AV_TIME_BASE : 0;
}
...
{
     toggle_mute((VideoState *) handle);
}

void vp_update_volume(vp_handle_t handle, int up)
{
    update_volume((VideoState *) handle, up ? 1 : -1, SDL_VOLUME_STEP);
}

void vp_step_to_next_frame(vp_handle_t handle)
{
    step_to_next_frame((VideoState *) handle);
}

void vp_cycle_channel_audio(vp_handle_t handle)
{
    stream_cycle_channel((VideoState *) handle, AVMEDIA_TYPE_AUDIO);
}

void vp_cycle_channel_video(vp_handle_t handle)
{
    stream_cycle_channel((VideoState *) handle, AVMEDIA_TYPE_VIDEO);
}

void vp_cycle_channel_subtitle(vp_handle_t handle)
{
    stream_cycle_channel((VideoState *) handle, AVMEDIA_TYPE_SUBTITLE);
}

void vp_seek_chapter(vp_handle_t handle, int incr)
{
    seek_chapter((VideoState *) handle, 1);
}
...
{
    VideoState *is = (VideoState *) handle;
    int seek_by_bytes = !!(is->ic->iformat->flags & AVFMT_TS_DISCONT) && strcmp("ogg", is->ic->iformat->name);

    if (seek_by_bytes > 0) {
        if (is->ic->bit_rate)
            pos *= is->ic->bit_rate / 8.0;
        else
            pos *= 180000.0;
        stream_seek(is, pos, 0, 1);
    } else {
        int64_t start = (is->ic->start_time != AV_NOPTS_VALUE) ? is->ic->start_time : 0;
        stream_seek(is, start + (int64_t)(pos * AV_TIME_BASE), 0, 0);
    }
}
